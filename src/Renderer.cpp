...
Renderer::Renderer(const Grid &grid, int scale)
  ...
  baseTex.create(grid.width, grid.height);
  baseSprite.setTexture(baseTex);
  baseSprite.setScale(static_cast<float>(scale), static_cast<float>(scale));

  compositeShader.loadFromMemory(COMPOSITE_GLSL, sf::Shader::Fragment);
...
void Renderer::draw(sf::RenderWindow &window) {
  ...
  // Keeps the shader simple (one texture sample) vs the old two-texture approach.
  for (int i = 0; i < grid.width * grid.height; ++i) {
    ...
    pixels[i4]     = static_cast<sf::Uint8>(std::clamp(r, 0.0f, 255.0f));
    pixels[i4 + 1] = static_cast<sf::Uint8>(std::clamp(g, 0.0f, 255.0f));
    pixels[i4 + 2] = static_cast<sf::Uint8>(std::clamp(b, 0.0f, 255.0f));
  }
...
sf::Color Renderer::cellColor(CellType type) {
  switch (type) {
  case CellType::AIR:       return {15, 15, 20, 255};
  case CellType::SAND:      return {212, 154, 74, 255};
  case CellType::WATER:     return {64, 164, 223, 200};
  case CellType::DIRT:      return {101, 67, 33, 255};
  case CellType::FIRE:      return {255, 80, 20, 255};
  case CellType::SMOKE:     return {180, 180, 180, 120};
  case CellType::GUNPOWDER: return {30, 30, 30, 255};
  case CellType::STONE:     return {120, 120, 120, 200};
  case CellType::ACID:      return {178, 255, 50};
